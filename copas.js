...
let fakeimage = fs.readFileSync("./media/WpCopas.png")
...
module.exports = copass = async (copass, mek) => {
	try {
		...
		const sender = mek.key.fromMe ? copass.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
	    ...
	    if (m.fromMe) return
            ...
            copass.reply(from, 'ViewOnce detected!', mek).then(() => copass.forwardMessage(from, message))
}
        ...
        const reply = (teks) => {
	      copass.sendMessage(from, teks, text, {quoted:mek, thumbnail: fakeimage})
        }
        const sendMess = (hehe, teks) => {
           copass.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? copass.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/WpCopas.png')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : copass.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/WpCopas.png')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           copass.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return copass.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/WpCopas.png')})
        }
        ...
        const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./media/CopasMage.jpg')
        }}}
       const math = (teks) => {
       ...
       const sendKontak = (from, nomor, nama, org = "") => {
	       ...
	       copass.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: mek})
}
      const hideTag = async function(from, text){
	       let anu = await copass.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       copass.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/CopasMage.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           copass.sendMessage(to, media, type, {quoted: mek, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       copass.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       copass.sendMessage(from, hasil, type, options).catch(e => {
	       copass.sendMessage(from, { url : link }, type, options).catch(e => {
		...
		const isQuotedSticker = type === 'extendedTextMessage' && content.includes('stickerMessage')
         ...
         // Sewa
           _sewa.expiredCheck(copass, sewa)
        
         // TicTacToe
              if (isTicTacToe(from, tictactoe)) tictac(prefix, tictactoe, from, sender, copass, mek)
        
             // Game
              game.cekWaktuFam(copass, family100)
              game.cekWaktuTG(copass, tebakgambar)
              game.cekWaktuMtk(copass, mtk)
              game.cekWaktuCkl(copass, ckl)
            ...
            if (anjuy.length < 1){
                copass.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
        ...
        if (isCmd && isGroup)
           ...
           case 'owner': case 'creator':
               ...
               prep = await copass.prepareMessageFromContent(from,{buttonsMessage},{})
               copass.relayWAMessage(prep)
               break      
           case 'menu': case 'help':
           let p = 0
               menu =`Yo Selamat ${waktu.charAt(0).toUpperCase() + waktu.slice(1)} @${sender.split('@')[0]} 👋\n
*Tanggal:* ${tanggal}
...
Adwijasing/Baileys
`
               copass.sendMessage(from, menu, text, {quoted: mek, contextInfo: {mentionedJid: [sender], externalAdReply: { title: 'Copas Team', thumbnailUrl: 'https://telegra.ph/file/ad408bff29a90b6627e6f.jpg', sourceUrl: 'https://github.com/Yudhaperdana/' }}})
               break
//------------------< Sticker Cmd >-------------------
...
//------------------< Game >-------------------
        case 'family100':
          ...
          case 'tebakgambar':
              ...
              copass.sendMessage(from, ini_buffer, image, { quoted: mek, caption: `Silahkan jawab soal berikut ini\n\nPetunjuk : ${petunjuk}\nWaktu : ${gamewaktu}s`})
        ...
        case 'tictactoe': case 'ttt': case 'ttc':
			  ...
			  if (mek.mentionedJid[0] === sender) return reply('Sad, main ama diri sendiri')
					...
					copass.reply(from, txt, mek, { contextInfo: { mentionedJid: copass.parseMention(txt) }})
...
//------------------< Downloader/Search/Anime >-------------------
          case 'nhentaisearch': case 'nhsearch':
              ...
              listM = copass.prepareMessageFromContent(from, { listMessage: { title: "NHENTAI BOT\n", description: "nHentai search", buttonText: "Click Here", listType: 1, sections: [{ rows: rowsdata }] }}, { quoted: mek })
              copass.relayWAMessage(listM, { waitForAck: true })
              break
...
🚀 Durasi : ${res[0].size}`
              ...
              imageMsg = ( await copass.prepareMessage(from, fs.readFileSync(`./ytmp.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Pilih satu format di bawah ini', imageMessage: imageMsg,
              contentText:`${result}`,buttons,headerType:4}
              prep = await copass.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              copass.relayWAMessage(prep)
          ...
          case 'tiktokdl': case 'tiktok':
              ...
              imageMsg = ( await copass.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Pilih satu format di bawah ini', imageMessage: imageMsg,
              contentText:`${result}`,buttons,headerType:4}
              prep = await copass.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              copass.relayWAMessage(prep)
          ...
          case 'nhentaipdf': case 'nhdl':
              ...
              copass.sendMessage(from, pdf, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${get_result.result.title_romaji}.pdf`, thumbnail: ini_image })
              break
          case 'buttons1':
              await axios.get(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=Nyarlathotep&q=${q}`)
		     .then(res => {
			  copass.sendMessage(from, { url: res.data.result.link }, 'audioMessage', { mimetype: 'audio/mp4', quoted: mek, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.source }}})
})
              break
          case 'buttons2':
              if (args.length < 1) return reply('Link Nya Mana?')
          ...
          case 'buttons3': 
              ...
              copass.sendMessage(from, ini_video, video, { quoted: mek })
              break
          case 'buttons4' :
              if (!q) return reply('Linknya?')
              if (!q.includes('tiktok')) return reply(mess.error.Iv)
          ...
          case 'google':
              ...
              copass.sendMessage(from, ss, image, {caption: vars, quoted : mek, thumbnail: Buffer.alloc(0) })
              }).catch(e => {
              console.log(e)
              reply(`${e}`)
})
...
_*Tunggu Proses Upload Media......*_`
          ...
          case 'yts': case 'ytsearch': 
			  ...
			  copass.sendMessage(from, capt.trim(), text, { contextInfo: { externalAdReply: { title: videos[0].title, body: videos[0].description, mediaType: 2, thumbnailUrl: videos[0].image, mediaUrl: videos[0].url }}})
			  break
          case 'tourl':
          ...
          case 'telesticker': case 'telestiker':
              ...
              copass.sendMessage(from, ini_buffer, sticker, {})
}
              break
          case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Nino`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              copass.sendMessage(from, buffer, sticker, { quoted: mek })
              break
          ...
          case 'tovideo':
              ...
              mediaaa = await copass.downloadAndSaveMediaMessage(encmediaaa)
              a = await webp2gifFile(mediaaa)
              mp4 = await getBuffer(a.result)
              copass.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: mek, caption: mess.success})
          ...
          case 'toimg':
              ...
              copass.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
          case 'nulis': case 'tulis':
              ...
              copass.sendMessage(from, buff, image, {quoted: mek, caption: mess.success}).catch(e => {
...
//------------------< Ingfo Bot >-------------------
          ...
          case 'bc': case 'broadcast':
              ...
              anu = await copass.chats.all()
              if (isMedia && !mek.message.videoMessage || isQuotedImage) {
              encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
              bc = await copass.downloadMediaMessage(encmedia)
              for (let _ of anu) {
              copass.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 Broadcast 」*\n\n${body.slice(4)}`})
}
              reply('Sukses')
              } else {
              for (let _ of anu) {
...
//------------------< Lainnya >-------------------
          case 'getpp':
              ...
              linkpp = await copass.getProfilePicture(from) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
              buffer = await getBuffer(linkpp)
              copass.sendMessage(from, buffer, image, {caption: "Nih", quoted: mek })
              } else if (mek.message.extendedTextMessage.contextInfo.mentionedJid === null || mek.message.extendedTextMessage.contextInfo.mentionedJid === undefined) {
              mberr = mek.message.extendedTextMessage.contextInfo.participant
              linkpp = await copass.getProfilePicture(mberr) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
              buffer = await getBuffer(linkpp)
              copass.sendMessage(from, buffer, image, { quoted: mek, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
          ...
          case 'searchmsg':  //by ANU TEAM
              ...
              copass.sendMessage(from, `Ditemukan!`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
              } catch (e) {
          ...
          case 'welcome':
              ...
              break
          case 'infoig':
              teks = `Jangan Lupa Follow Ig Owner Ya : https://www.instagram.com/copass.chan26/`
              copass.sendMessage(from, teks, text, { quoted : mek })
              break
          ...
          case 'q':
              ...
              let qu = copass.serializeM(await m.getQuotedObj())
...
if (!isOwner) return
try {
return copass.sendMessage(from, 
